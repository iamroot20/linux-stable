...
/* IAMROOT20 20240309 
 * ex) PHYS_ADDR_MAX = 0xffff_ffff_ffff_ffff
       base = 0xffff_ffff_ffff_fff0
       size = 0x100
       return size = min(0x100, 0xf) = 0xf;
   (base + size)가 overflow가 발생하면 (PHYS_ADDR_MAX - base)를 size로 재설정
 */
 ...
 */
/* IAMROOT20 20240309
 * (start_rgn - 1) index부터 end_rgn index미만까지 merge
 */
						   ...
						   unsigned long end_rgn)
{
	...
	while (i < end_rgn) {
		...
		if (this->base + this->size != next->base ||
			...
			/* IAMROOT20 20240309
			 * this region과 next region이 인접하지 않거나 같은 node가 아니면 merge할 수 없으므로 다음 region을 확인하러 넘어감
			 */
		...
		/* IAMROOT20 20240309
		 * this region과 next region을 merge
		 */
		this->size += next->size;
		/* move forward from next + 1, index of which is i + 2 */
		/* IAMROOT20 20240309
		 * (next + 1)부터 (type->cnt - (i + 2))개를 앞으로 한칸씩 이동시킴 
		 */
 ...
 */
/* IAMROOT20 20240309 
 * (base ~ size) 영역을 memblock->type->region[idx] 위치에 insert
 */
						   ...
						   enum memblock_flags flags)
{
	...
	/* IAMROOT20 20240309
	 * memblock->type->region[idx]부터 뒤로 한 칸씩 이동시킴
	 */
	memmove(rgn + 1, rgn, (type->cnt - idx) * sizeof(*rgn));
	/* IAMROOT20 20240309
	 * memblock->type->region[idx] 위치에 new region을 insert
	 */
	rgn->base = base;
	rgn->size = size;
	rgn->flags = flags;
 ...
 */
/* IAMROOT20_START 20240309 */
/* IAMROOT20 20240309 
 * 참고: http://jake.dothome.co.kr/memblock-1/
 */
				...
				int nid, enum memblock_flags flags)
{
	...
	/* IAMROOT20 20240309
	 * type에 등록된 regions이 없을 경우
	 * new region을 등록 후 함수 종료
	 */
	 ...
	 */
	/* IAMROOT20 20240309
	 * 현재 최대로 넣을 수 있는 region 갯수는 type->max인데
	 * 최악의 경우인 (type->cnt * 2 + 1)보다 type->max가 작거나 같으면 insert를 true로 설정하여 repeat을 방지함 
	 */
	if (type->cnt * 2 + 1 <= type->max)
		insert = true;

/* IAMROOT20 20240309
 * insert가 false이면 먼저 정확한 new_region이 필요한 영역을 확인하고 insert를 true로 변경
 * insert가 true이면 repeat을 다시 실행하지 않고 실제 new region을 insert
 */
repeat:
	/*
	...
	for_each_memblock_type(idx, type, rgn) {
		...
		/* IAMROOT20 20240309
		 * (rbase >= end)일 경우 더 이상 new region이 기존 region과 겹치지 않으므로 반복문 종료
		 */
		if (rbase >= end)
			break;
		/* IAMROOT20 20240309
		 * (rend <= base)일 경우 new region의 현재 비교하고 있는 region과 겹치지 않으므로 다음 region으로 진행
		 */
		if (rend <= base)
			continue;
		/*
		 ...
		 */
		/* IAMROOT20 20240309
		 * (rbase > base)일 경우 new region의 하위 부분과 현재 비교하고 있는 region이 겹치므로 new region을 하위 부분인 (base ~ rbase) 영역를 insert
		 */
		if (rbase > base) {
#ifdef CONFIG_NUMA
	...
	/* IAMROOT20 20240309
	 * insert 해야하는 new region이 없으면 함수 종료
	 */
	if (!nr_new)
		return 0;
	...
	if (!insert) {
		/* IAMROOT20 20240309
		 * insert 해야될 new region이 max region size보다 크면 memblock_double_array를 호출하여 memblock.type의 사이즈를 2배 증가시킴
		 */
		...
		/* IAMROOT20 20240309
		 * 모든 insert 작업이 끝나고 인접한 region들끼리 merge를 수행
		 */
